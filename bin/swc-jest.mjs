...
const commands = ['jest', 'defaults', 'watch']

if (!script) {
  console.log('swc-jest: No command provided')
  process.exit(1)
}

if (!commands.includes(script)) {
  console.log(`swc-jest: Unknown command "${script}"`)
  process.exit(1)
}
...
if (['SIGKILL', 'SIGTERM'].includes(result.signal)) {
  console.error('swc-jest: The command failed because the process exited too early')
  process.exit(1)
}
